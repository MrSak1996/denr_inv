...
const fetchTransaction = async () => {
    try {
        ...
        const response = await api.get(`/fetchTransaction?api_token=${api_token}`)
        loading.value = false
                    ...
                    <div
                        ...
                        <div class="flex flex-col justify-center items-center gap-x-2 py-6 px-4">
                            ...
                            <div class="w-full bg-gray-200 rounded-full h-4">
                                <div class="bg-teal-500 h-4 rounded-full transition-all"
                ...
                <DataTable size="small" v-model:filters="filters" :value="trans_logs" paginator showGridlines :rows="10"
                    ...
                    <Column field="transaction_type" header="Transaction Type" style="min-width: 12rem">
                        ...
                        <template #filter="{ filterModel }">
                            <InputText v-model="filterModel.value" type="text" placeholder="Search by transaction" />
                        </template>
                    </Column>

                    <Column field="qr_code" header="QR Code" style="min-width: 8rem">
                        <template #body="{ data }">
                            <QrcodeVue v-if="data.qr_code && data.qr_code.trim() !== ''" :value="data.qr_code"
                    ...
                    <Column field="source_location" header="Source Location" style="min-width: 5rem">
                        ...
                        <template #filter="{ filterModel }">
                            <InputText v-model="filterModel.value" type="text" placeholder="Search by source location" />
                        </template>
                    </Column>
